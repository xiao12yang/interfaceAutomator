...
class RequestsBase:
        ...
        for _ in range(yaml_data_str.count('${')):
            if '${' in yaml_data_str and '}' in yaml_data_str:
                ...
                if match:
                    ...
                    # yaml_data_str = yaml_data_str.replace('\"','')
    ...
    def execute_test_case(self, api_info):
        ...
        try:
            ...
            headers = api_info['baseInfo'].get('headers')
            ...
            cookies = api_info['baseInfo'].get('cookies')
            ...
            # 这里为了避免用例运行失败重跑时出现数据丢失，所以用变量临时存储api_info['testCases']，这里使用深拷贝
            testCases = copy.deepcopy(api_info['testCases'])
            for testcase in testCases:
                ...
                # 通过变量引处理断言结果
                validation = self.parses_replace_variables(testcase.get('validation'))
                testcase.pop('validation')
                # 处理接口返回值提取部分
                ...
                for param_type, param_value in testcase.items():
                    ...
                    if param_type in ['data', 'params', 'json']:
                        testcase[param_type] = self.parses_replace_variables(param_value)
                        allure.attach(f'参数类型：{param_type}\n{self.allure_attach_dict_response(testcase[param_type])}', f'参数类型：{param_type}', attachment_type=allure.attachment_type.TEXT)
